...
def complete_trade(request, room_code):
    ...
    try:
        with transaction.atomic():
            ...
            # 🔥 중요: room.save()가 시그널을 발생시켜 자동으로 알림 생성됨
            room.save()
    ...
    if is_fully_completed:
        ...
        # 한쪽만 완료 - 상대방에게 실시간 UI 업데이트용 WebSocket 전송
        # 🔥 알림은 시그널에서 자동 생성됨
        async_to_sync(channel_layer.group_send)(
            f"chat_{room.room_code}",
            {
    ...
    return JsonResponse({
        ...
        'post_updated': post_updated,
        'message': f'{"구매자" if user_role == "buyer" else "판매자"} 거래완료 처리되었습니다.',
        # 🔥 프론트엔드 즉시 UI 업데이트용 데이터
        'ui_update': {
            'status_text': '상대방의 완료를 기다리는 중' if not is_fully_completed else '거래 완료됨',
            'status_class': 'waiting bg-purple-100 text-purple-800' if not is_fully_completed else 'completed bg-green-100 text-green-800',
            'hide_complete_button': True
        }
    })